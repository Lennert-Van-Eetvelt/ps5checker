...
function main(){
    ...
    switch(place){
        ...
        case 10: checkAmazon();return;
        case 11: place = 0; main(); return;
    }
}
...
function checkBol(){
    ...
    if (!window.location.href.startsWith(urlBase)) goToUrl(urlFull, 3898); else{
    let ele = document.getElementsByClassName("buy-block__title")[0];
    if (!ele.innerHTML.startsWith("Niet")) found("bol",urlFull);
...
function checkAlternateDisc(){
    ...
    if (!window.location.href.startsWith(urlFull)) goToUrl(urlFull, 3898); else{
    let ele = document.getElementsByClassName("d-flex justify-content-center align-items-center")[0].children[0];
    if (!ele.innerHTML.startsWith("Niet")) found("alternate", urlFull);
    else console.log("not found....")
    place++;
    savePlace();
    main();}
...
function checkCoolblueAll(number, url){
    ...
    try {
        ...
        if (ele.innerHTML.startsWith("Tijdelijk")) available = false;
    }catch (E){
        console.log(e)
    }
...
function checkAmazonDigital(){
    ...
    try {
        ...
        if (ele.innerHTML.startsWith("Currently")) available = false;
    }catch (E){
        console.log(e)
    }
